...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Version-v0.15.0_(11d9309)-purple?style=for-the-badge" alt="Version" height="28">
...
- [2026-05-28-update](changelog/2026-05-28-update.md): **90 commits 跨日同步 + v0.15.0 发布**（hermes-agent `963d22c → 11d9309`）— **v0.15.0 "The Velocity Release"（v2026.5.28）正式打标**（`pyproject.toml`/`hermes_cli/__init__.py` → `0.15.0`，自 v0.14.0 累计 1,302 commits / 747 PRs，多数 highlight 已于 05-22~05-27 日同步覆盖）、**claude-opus-4.8 / -fast 接入**（`models.py:35-36,144,296` + `model_metadata.py:144-145` 1M ctx + `anthropic_adapter.py:80,85,89` 继承 Opus 4.7 API 契约；fast 为独立 model ID 2× 定价非 speed 参数）、**Memory Provider 主机契约扩展**（`sync_turn` 加 `messages` kwarg 签名内省派发 + Hindsight `recall_types` 默认 observation-only `hindsight/__init__.py:590`）、**Context Engine 外部引擎主机契约**（`run_agent.py:530 _transition_context_engine_session` + conversation_id 转发 + cache 桶转发，凝练 5 PR #33750）、**Kanban SQLite 抗损坏 wave**（`kanban_db.py:1305` synchronous=FULL + `:1310` secure_delete + `:1313` cell_size_check + page_count 不变量 `:1633` + Windows init lock `:1023` + content-addressed 备份 + FD 关闭 + grace period + zombie reaper hoist ~14 commit）、**Skills 目录全量拉取**（skills.sh sitemap 858→19,932 `skills_hub.py:1215-1363` + ClawHub 200→20k+ nextCursor 分页 + 34MB 懒加载）、**web_crawl 工具整体移除**（25 文件 +115/-1067，crawl 能力下线 #33824）、**MCP mTLS 客户端证书**（`mcp_tool.py:562 _resolve_client_cert`，HTTP/SSE，三 schema 形式 #33721）、**Docker wave 7 连**（git SHA 烤入镜像 + docker pull 指引 + HOME 对齐 + python-is-python3 + s6 受监管重定向）、**安全加固**（tirith/android tar-member 拒绝 + msgraph webhook source CIDR 白名单 `msgraph_webhook.py:67,136` + API_SERVER_KEY 强制 `api_server.py:4105` + npm overrides 钉版）、**Krea 2 Medium/Large 入 FAL 目录**（`image_generation_tool.py:327,349` #33506）、**Agent/Provider 健壮性**（content-policy 立即 fallback + retry 状态缓冲仅终态上屏 + openrouter session_id sticky + /model 与 hermes model 列表统一）、**Nous Portal entitlement 归一化 + Codex 池 re-auth 同步**、**xAI proxy 429 + OAuth bare-code paste + timeout fallback**、**Discord 线程回填三连**、**dashboard 可折叠侧边栏 + 移动端滚动 + stale-token SPA 重载**、文档站 30 天大检修（#33782）
...
- **更新日志**: 32 个
- **源码覆盖**: 关键模块逐行验证
- **跟踪版本**: v0.15.0（hermes-agent/main HEAD `11d9309`）
- **最后更新**: 2026-05-28
