...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Version-v0.15.1_(5921d6678)-purple?style=for-the-badge" alt="Version" height="28">
...
- [2026-05-30-update](changelog/2026-05-30-update.md): **95 commits 跨日同步**（hermes-agent `689ef5e2 → 5921d6678`，v0.15.1 post-release）— **Kanban 任务文件附件**（`hermes_cli/kanban_db.py:399-2547` + REST `/tasks/{id}/attachments` 25 MB cap，#35395）、**`/compress here [N]` 边界对齐部分压缩**（灵感 Claude Code v2.1.139 "Summarize up to here"，`hermes_cli/partial_compress.py` 235 行 + seam 守卫，#35048）、**`hermes prompt-size` 离线 prompt 预算诊断**（`hermes_cli/prompt_size.py:52 compute_prompt_breakdown`，零网络 + 零模型工具足迹，#35276）、**Model picker 多端点 provider 聚合**（`hermes_cli/models.py:958 PROVIDER_GROUPS` + `:979 group_providers()`，Kimi/MiniMax/xAI/Google/OpenAI/OpenCode/Copilot 折叠，DISPLAY ONLY，#35227）、**WhatsApp + WeChat 文本去抖批合并**（`gateway/platforms/{whatsapp,weixin}.py`，config.extra-only，#35301）、**`read_file` 紧凑行号 gutter ~14% token 节省**（`tools/file_operations.py:705-730`，`HERMES_READ_GUTTER=padded` 回归旧格式，#35368）、**`write_file`/`patch` 原子写**（same-dir temp + `mv`，POSIX 原子，crash-safe，#35252）、**MCP 启动期非阻塞化**（`hermes_cli/mcp_startup.py` daemon 后台 + 0.75s 短等，TUI 启动 `~7500ms → ~115ms`）、**Starlette CVE-2026-48710 BadHost pin** `starlette==1.0.1`（`pyproject.toml:86/118/125/180` + `tools/lazy_deps.py:176`，#35118）、**mid-session `/model` 切换持久化**（`hermes_state.py:968-980 update_session_model`，fixes #34850）、**TUI `/agents` 看板 nudge**（`display.tui_agents_nudge` default true，`createGatewayEventHandler.ts:143-186`，#34704+#35216）、**进程名 = `hermes`**（`setproctitle` 优先 + Linux `prctl(PR_SET_NAME)` + macOS `pthread_setname_np` fallback，#35108）、UTF-8 BOM handling（`read_file`/`write_file`/`patch`，#35278）、Compressor resume/handoff 清理 + post-interrupt 状态恢复
