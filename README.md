...
- [messaging-gateway-architecture](concepts/messaging-gateway-architecture.md): 20+ 平台统一网关（v0.13.0 起含 Google Chat/Teams/MS Graph + IRC/腾讯元宝/微信/QQ Bot），平台适配器插件化（PlatformRegistry）、`allowed_*` 白名单、`[[as_document]]`、auto-resume、代理模式
...
- [2026-05-09-update](changelog/2026-05-09-update.md): 1100 commits (v0.13.0 + post-release)，**ProviderProfile 插件化（28 个 bundled provider）**、**Kanban 持久化多 Agent 看板**、`/goal` Ralph 循环、Checkpoints v2、会话自动恢复、Google Chat / Teams / MS Graph 三平台、SearXNG/Brave/DDGS 搜索、Lightpanda 浏览器引擎、cron `no_agent` 模式、5 个新 hook、8 个 P0 安全闭环、i18n 7 语言、Windows beta
...
- **更新日志**: 6 个
- **源码覆盖**: 关键模块逐行验证
- **跟踪版本**: v0.13.0 (v2026.5.7) + post-release（HEAD `bfc84bdc6`）
- **最后更新**: 2026-05-09
