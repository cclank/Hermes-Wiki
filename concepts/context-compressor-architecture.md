...
class ContextCompressor:
    def __init__(self, model, threshold_percent=0.50):
        ...
        self.max_summary_tokens = min(context_length * 0.05, 16384)  # 摘要上限
```
...
def _compute_summary_budget(turns_to_summarize):
    ...
    return max(512, min(budget, self.max_summary_tokens))
```
...
- [[model-metadata-and-routing]] — 使用 get_model_context_length() 获取上下文窗口
