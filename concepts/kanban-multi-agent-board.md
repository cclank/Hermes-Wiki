...
>
> **2026-05-30 更新（hermes-agent `5921d6678`，#35395）— 任务文件附件**：任务可携带 PDF / 图像 / 源文档等附件，worker 按绝对路径直接 `read_file` / `pdftotext`，关掉"把源材料粘贴成 path 塞 body"的旧路径。
>
> - **DB**（`hermes_cli/kanban_db.py`）：`task_attachments` 表（`:1015`，additive 迁移）+ `Attachment` 数据类（`:870`）+ `add/list/get/delete_attachment` 访问器（`:2487/2509/2529/2547`）；`attachments_root(board)` / `task_attachments_dir(task_id, board)` 路径 helper（`:399 / :429`），`HERMES_KANBAN_ATTACHMENTS_ROOT` 环境变量绝对路径覆盖；default board 用 `<root>/kanban/attachments/`，其他 board 用 `<root>/kanban/boards/<slug>/attachments/`
> - **worker context**（`kanban_db.py:6650-6668 build_worker_context`）：附件以 "Files attached to this task. Read them with the file/terminal tools at the absolute paths below" 引导，按 `- \`<filename>\`, <ctype>, <size> KB → \`<absolute_path>\`` 行格式 surface；本地 terminal backend 直接解析，Docker / Modal 等远端 backend 需挂载 attachments 目录
> - **Dashboard REST**（`plugins/kanban/dashboard/plugin_api.py`）：`GET / POST / DELETE /tasks/{task_id}/attachments`（multipart 上传），`_MAX_ATTACHMENT_BYTES = 25 * 1024 * 1024`（`:636`），`_safe_attachment_name()` 防 traversal（`:639-655`），下载侧 root-containment 校验
> - **Dashboard UI**：task drawer 新增 Attachments 区，上传按钮 + 列表 + 行内 download / remove
> - **测试**：`tests/plugins/test_kanban_attachments.py`（291 行，13 用例：DB accessor / REST 往返 / traversal 拒绝 / 文件名冲突后缀 / worker-context surface），关闭 #35338
>
> 详见 [[2026-05-30-update#1-kanban-任务文件附件35395]]。
>
...
- `task_runs` —— 每次尝试历史
- `task_attachments`（2026-05-30，#35395，`:1015`）—— 任务文件附件元数据；blob 落盘到 `attachments_root(board)/<task_id>/<stored_name>`
