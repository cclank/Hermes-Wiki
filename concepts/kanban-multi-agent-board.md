...
### SQLite 抗损坏加固 wave（2026-05-28，v0.15.0）

针对生产环境第 6 次 DB 损坏（Bug E：b-tree 页头清零、旧 cell 残留），一组 ~14 commit 把持久层打到可检测/可恢复。每次 `connect()` 应用三条 pragma（`6416dd518`）：

| pragma | 行 | 作用 |
|--------|----|----|
| `PRAGMA synchronous=FULL`（原 NORMAL） | `kanban_db.py:1305` | 关闭 WAL-checkpoint 重排窗口；本地 SSD <1ms/commit |
| `PRAGMA secure_delete=ON` | `:1310` | 释放页字节清零，损坏可检测、旧行不复活为活数据；首连写持久标志入 header |
| `PRAGMA cell_size_check=ON` | `:1313` | 读侧守卫，坏 cell 在读时报错而非静默返回错数据 |

其余健壮性修复：
- **`page_count` 不变量**（`99c19eb2f`）：`write_txn` 加 post-commit 检查（`:1633` `_check_page_count_invariant` / `:1697` 调用点）—— SQLite header 声明页数 vs 实际文件页数不匹配即报错。
- **Windows 初始化锁**（`3ba896273`）：`_cross_process_init_lock`（`:1023`）跨进程串行化首连 WAL/schema/integrity 探测，正常使用仍并发。
- **content-addressed 损坏备份**（`6f9182cb3`）：损坏 DB（含 WAL/SHM sidecar）按内容寻址命名备份（`:1142`）。
- **crash grace period**（`c002668ff`）：`detect_crashed_workers` 加宽限期（`_resolve_crash_grace_seconds` `:147`）。
- **WAL 不降级**（`5c49cd0ed`）：transient EIO 下永不静默把 WAL 降级为 DELETE。
- **FD 关闭**（`ebe04c66c`）：长生命周期进程每次 `connect()` 后关闭 kanban.db FD。
- **冗余 pragma 跳过**（`dc98314fb`）：已是 WAL 的连接跳过重复 WAL pragma（`:1260`）。
- **zombie reaper hoist**（`ffdc937c1`）：从 `dispatch_once` 提出，独立调度。
- **连接并发加固**（`90b6b3d18`）、**rollback 保留原始异常**（`e83252dc4`）、**SIGTERM 终止 worker 进程**（`f30db14ce`，#28181）。

UI：列改横向滚动条而非换行（`70abae8e3`）/ 列折行成多行修垂直溢出（`538f0fa33`）。详见 [[2026-05-28-update]]。
