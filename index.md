...
> 查询前先读此文件找到相关页面。
> Last updated: 2026-05-28 | Total pages: 46 concepts + 2 entities + 32 changelogs | Tracking: v0.15.0 (hermes-agent/main HEAD `11d9309`)
...
### 更新日志（按时间倒序）
- [[2026-05-28-update]] — **90 commits 跨日同步 + v0.15.0 发布**（`963d22c → 11d9309`）：**v0.15.0 "The Velocity Release" 正式打标**（自 v0.14.0 累计 1,302 commits / 747 PRs，多数 highlight 已于既往日同步覆盖）+ **claude-opus-4.8 / -fast 接入**（1M ctx，继承 Opus 4.7 API 契约，fast 为独立 ID 2× 定价）+ **Memory Provider 主机契约扩展**（`sync_turn` messages kwarg + Hindsight recall_types observation-only）+ **Context Engine 外部引擎主机契约**（`_transition_context_engine_session` 凝练 5 PR）+ **Kanban SQLite 抗损坏 wave**（synchronous=FULL + secure_delete + cell_size_check + page_count 不变量 + Windows init lock + content-addressed 备份 ~14 commit）+ **Skills 目录全量拉取**（skills.sh sitemap 858→19,932 + ClawHub 200→20k+）+ **web_crawl 工具整体移除**（crawl 能力下线，25 文件 -1067）+ **MCP mTLS 客户端证书**（HTTP/SSE）+ **Docker wave 7 连** + **安全加固**（tar-member 拒绝 + msgraph CIDR 白名单 + API_SERVER_KEY 强制 + npm overrides）+ **Krea 2 Medium/Large 入 FAL** + **Agent/Provider 健壮性**（content-policy 立即 fallback + openrouter sticky + /model 列表统一）+ **xAI proxy/OAuth 修复** + **Discord 线程回填三连** + **dashboard 可折叠侧边栏**
